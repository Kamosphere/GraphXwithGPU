...
(@transient override val vertices: VertexRDD[VD],
 @transient override val replicatedVertexView: ReplicatedVertexView[VD, ED])
  ...
  def this(g: Graph[VD, ED]) =
    this(g.vertices,
      new ReplicatedVertexView(g.edges.asInstanceOf[EdgeRDDImpl[ED, VD]], true, true))

  def messageUpdateExtract
  (activeSetOpt: Option[(VertexRDD[VD], EdgeDirection)]): RDD[EdgeTriplet[VD, ED]] = {
